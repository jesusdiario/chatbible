    ...
    <>
      ...
      >
        <nav className="flex h-full w-full flex-col px-3" aria-label="Histórico de Conversas">
                    ...
                    {timeframes.map(timeframe => (
                      <div key={timeframe.title}>
                        ...
                        {timeframe.items.map(item => (
                          <div key={item.id} className="group flex h-10 items-center gap-2.5 rounded-lg px-2 hover:bg-token-sidebar-surface-secondary cursor-pointer" onClick={() => onChatSelect?.(item.id)}>
          ...
          {isOpen && (
            <div className="flex flex-col py-2 border-t border-white/20 bg-chatgpt-sidebar">
              <button 
