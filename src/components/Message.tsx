...
import { useAudio } from '@/hooks/useAudio'; // Novo hook para áudio
...
const Message: FC<MessageProps> = ({ role, content, isTyping = false }) => {
  const { playAudio, isPlaying, stopAudio } = useAudio(); // Hook para reprodução de áudio
        ...
        <div className={`flex-1 space-y-2 ${role === 'user' ? 'flex justify-end' : ''}`}>
          ...
          >
            {role === 'user' ? (
              ...
              <>
                ...
                {isTyping && <span className="inline-block w-2 h-4 ml-1 -mb-1 bg-white animate-blink" />}
              </>
            )}
          </div>
          {role === 'assistant' && !isTyping && (
            <div className="flex items-center gap-2">
              ...
              {/* Botão de áudio para mensagens do assistente */}
              {role === 'assistant' && !isTyping && content && (
                <button
                  onClick={() => (isPlaying ? stopAudio() : playAudio(content))}
