...
const ExegeseCapitulo = () => {
  ...
  useEffect(() => {
    const savedHistory = localStorage.getItem('exegeseHistory');
    const savedChatsData = localStorage.getItem('exegeseChatsData');
    if (savedHistory) {
  ...
  useEffect(() => {
    if (chatHistory.length > 0) {
      localStorage.setItem('exegeseHistory', JSON.stringify(chatHistory));
    }
    if (Object.keys(chatsData).length > 0) {
      localStorage.setItem('exegeseChatsData', JSON.stringify(chatsData));
    }
  ...
  const saveCurrentChat = (chatId: string, messageList: Message[]) => {
    ...
    const chatTitle = firstUserMessage ? firstUserMessage.content.substring(0, 30) + (firstUserMessage.content.length > 30 ? '...' : '') : 'Nova exegese';
    const now = new Date();
      ...
      <main className={`flex-1 transition-all duration-300 ${isSidebarOpen ? 'ml-64' : 'ml-0'}`}>
        ...
        <div className={`flex h-full flex-col ${messages.length === 0 ? 'items-center justify-center' : 'justify-between'} pt-[60px] pb-4`}>
          {messages.length === 0 ? <div className="w-full max-w-3xl px-4 space-y-4">
              <div>
                <h1 className="mb-8 text-4xl font-semibold text-center">Exegese de Capítulo</h1>
...
export default ExegeseCapitulo;
