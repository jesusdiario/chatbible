...
import { BookGrid } from "@/components/BibliaBookGrid";
...
const TestamentSection: React.FC<{ testament: any }> = ({ testament }) => {
  return (
    <div className="mb-8">
      <h2 className="text-xl font-semibold mb-4 border-b pb-2">{testament.name}</h2>
      <BookGrid books={testament.books} />
    </div>
  );
};
...
const Biblia = () => {
  ...
  const { testaments, isLoading, isError, error } = useBibliaData();
    ...
    <div className="flex flex-col md:flex-row h-screen">
      ...
      <main className={`flex-1 transition-all duration-300 ${isSidebarOpen ? 'ml-0 md:ml-64' : 'ml-0'}`}>
        ...
        <div className="pt-20 pb-6 px-4 bg-gray-50 text-dark min-h-screen">
          <div className="max-w-7xl mx-auto">
            <h1 className="text-3xl md:text-4xl font-bold mb-6 text-center">Bíblia Sagrada</h1>
            
            {testaments.map((testament) => (
              <TestamentSection key={testament.id} testament={testament} />
            ))}
          </div>
        </div>
      </main>
