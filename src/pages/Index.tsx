...
const Index = () => {
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      if (!slug) navigate(`/chat/${result.slug}`);
      ...
      setMessages(prev => [
        ...prev,
        { role: "assistant", content: "Ocorreu um erro: " + (err?.message || "Erro inesperado") }
      ]);
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="flex h-screen flex-col md:flex-row">
      <ErrorBoundary>
        ...
        <main className={`flex-1 transition-all duration-300 ${isSidebarOpen ? "ml-0 md:ml-64" : "ml-0"}`}>
          <ChatHeader isSidebarOpen={isSidebarOpen} onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
